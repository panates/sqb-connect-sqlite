...
proto.refresh = function(db, callback) {
  ...
  waterfall([
    ...
    function(next) {
      db.delete('sqb$all_columns').execute(next);
    },

    /* Empty sqb$all_constraints temporary table */
    function(next) {
      db.delete('sqb$all_constraints').execute(next);
    },
...
proto.queryColumns = function(db) {
  ...
  const self = this;
  query.hook('execute', function(callback) {
